...
+ (BOOL)pureMode {
    return [[NSUserDefaults standardUserDefaults] boolForKey:@"pure_mode"];
}
+ (BOOL)antiBurnIn {
    return [[NSUserDefaults standardUserDefaults] boolForKey:@"anti_burn_in"];
}
+ (BOOL)skipDownloadSelector {
    return [[NSUserDefaults standardUserDefaults] boolForKey:@"skip_download_selector"];
}
+ (BOOL)videoViewsCount {
    return [[NSUserDefaults standardUserDefaults] boolForKey:@"video_views_count"];
}
+ (BOOL)autoStreakReminder {
    return [[NSUserDefaults standardUserDefaults] boolForKey:@"auto_streak_reminder"];
}
+ (BOOL)commentSortTime {
    return [[NSUserDefaults standardUserDefaults] boolForKey:@"comment_sort_time"];
}
+ (BOOL)customRegionEnabled {
    return [[NSUserDefaults standardUserDefaults] boolForKey:@"en_custom_region"];
}
+ (NSString *)customRegionCode {
    NSString *val = [[NSUserDefaults standardUserDefaults] stringForKey:@"custom_region_code"];
    return val && val.length > 0 ? [val uppercaseString] : @"US";
}
+ (NSString *)customMcc {
    NSString *val = [[NSUserDefaults standardUserDefaults] stringForKey:@"custom_region_mcc"];
    return val && val.length > 0 ? val : @"310";
}
+ (NSString *)customMnc {
    NSString *val = [[NSUserDefaults standardUserDefaults] stringForKey:@"custom_region_mnc"];
    return val && val.length > 0 ? val : @"00";
}
+ (void)cleanCache {
...
+ (void)showSaveVC:(id)item {
    if ([BHIManager skipDownloadSelector]) {
        NSArray *items = [item isKindOfClass:[NSArray class]] ? (NSArray *)item : @[item];
        for (id singleItem in items) {
            NSURL *url = [singleItem isKindOfClass:[NSURL class]] ? (NSURL *)singleItem : [NSURL fileURLWithPath:[singleItem description]];
            NSString *ext = [[url pathExtension] lowercaseString];
            [BHIManager saveMedia:url fileExtension:ext];
        }
        JGProgressHUD *hud = [JGProgressHUD progressHUDWithStyle:JGProgressHUDStyleDark];
        hud.indicatorView = [[JGProgressHUDSuccessIndicatorView alloc] init];
        hud.textLabel.text = @"Saved to Photos!";
...
+ (void)renewAllStreaksWithCompletion:(void(^)(BOOL success, NSInteger count))completion {
    [[NSUserDefaults standardUserDefaults] setDouble:[[NSDate date] timeIntervalSince1970] forKey:@"bht_last_streak_renew_timestamp"];
    [[NSUserDefaults standardUserDefaults] synchronize];

    // Trigger internal IM streak renewal
    Class imManagerClass = NSClassFromString(@"AWEIMManager");
    if (imManagerClass && [imManagerClass respondsToSelector:@selector(sharedInstance)]) {
        id imManager = [imManagerClass performSelector:@selector(sharedInstance)];
        if ([imManager respondsToSelector:@selector(syncConversations)]) {
            [imManager performSelector:@selector(syncConversations)];
        }
    }

    [[NSNotificationCenter defaultCenter] postNotificationName:@"BHTikTokRenewStreaksNotification" object:nil];
