...
+ (void)showIfNeededOnView:(UIView *)parentView;
@end

@implementation BHTStreakBannerView

+ (void)showIfNeededOnView:(UIView *)parentView {
    if (!parentView) return;
    if (![BHIManager autoStreakReminder]) return;
    
    NSTimeInterval lastTime = [[NSUserDefaults standardUserDefaults] doubleForKey:@"bht_last_streak_banner_timestamp"];
    NSTimeInterval now = [[NSDate date] timeIntervalSince1970];
    if (now - lastTime < 6 * 3600) {
        return;
    }
    [[NSUserDefaults standardUserDefaults] setDouble:now forKey:@"bht_last_streak_banner_timestamp"];
    
    if ([parentView viewWithTag:7788]) return;
    
    CGFloat bannerWidth = parentView.bounds.size.width - 24;
    if (bannerWidth < 280) bannerWidth = 280;
    if (bannerWidth > 420) bannerWidth = 420;
    
    BHTStreakBannerView *banner = [[BHTStreakBannerView alloc] initWithFrame:CGRectMake((parentView.bounds.size.width - bannerWidth) / 2.0, -90, bannerWidth, 64)];
    banner.tag = 7788;
    [parentView addSubview:banner];
    
    [UIView animateWithDuration:0.5 delay:0.1 usingSpringWithDamping:0.75 initialSpringVelocity:0.5 options:UIViewAnimationOptionCurveEaseOut animations:^{
        banner.frame = CGRectMake((parentView.bounds.size.width - bannerWidth) / 2.0, 52, bannerWidth, 64);
    } completion:nil];
}
...
- (instancetype)initWithFrame:(CGRect)frame {
    ...
    if (self) {
        self.backgroundColor = [UIColor colorWithRed:0.12 green:0.12 blue:0.14 alpha:0.96];
        self.layer.cornerRadius = 16.0;
        self.layer.borderWidth = 1.2;
        self.layer.borderColor = [UIColor colorWithRed:1.0 green:0.35 blue:0.15 alpha:0.8].CGColor;
        ...
        self.titleLabel.text = @"🔥 Нужно продлить огоньки! Продлить?";
        ...
        self.renewButton = [UIButton buttonWithType:UIButtonTypeSystem];
        [self.renewButton setTitle:@"Продлить" forState:UIControlStateNormal];
        [self.renewButton setTitleColor:[UIColor whiteColor] forState:UIControlStateNormal];
        self.renewButton.titleLabel.font = [UIFont systemFontOfSize:13.0 weight:UIFontWeightBold];
        self.renewButton.backgroundColor = [UIColor colorWithRed:1.0 green:0.25 blue:0.15 alpha:1.0];
        self.renewButton.layer.cornerRadius = 14.0;
        self.renewButton.translatesAutoresizingMaskIntoConstraints = NO;
        [self.renewButton addTarget:self action:@selector(handleRenewTapped) forControlEvents:UIControlEventTouchUpInside];
        ...
        [self.closeButton addTarget:self action:@selector(dismissBanner) forControlEvents:UIControlEventTouchUpInside];
        [self addSubview:self.closeButton];
        
        [NSLayoutConstraint activateConstraints:@[
            [self.titleLabel.leadingAnchor constraintEqualToAnchor:self.leadingAnchor constant:12],
            [self.titleLabel.centerYAnchor constraintEqualToAnchor:self.centerYAnchor],
            [self.titleLabel.trailingAnchor constraintEqualToAnchor:self.renewButton.leadingAnchor constant:-8],
            
            [self.renewButton.trailingAnchor constraintEqualToAnchor:self.closeButton.leadingAnchor constant:-6],
            [self.renewButton.centerYAnchor constraintEqualToAnchor:self.centerYAnchor],
            [self.renewButton.widthAnchor constraintEqualToConstant:84],
            [self.renewButton.heightAnchor constraintEqualToConstant:32],
            
            [self.closeButton.trailingAnchor constraintEqualToAnchor:self.trailingAnchor constant:-10],
...
- (void)handleRenewTapped {
    UINotificationFeedbackGenerator *gen = [[UINotificationFeedbackGenerator alloc] init];
    [gen notificationOccurred:UINotificationFeedbackTypeSuccess];
    
    UIView *targetView = self.superview;
    [BHIManager renewAllStreaksWithCompletion:^(BOOL success, NSInteger count) {
        JGProgressHUD *hud = [JGProgressHUD progressHUDWithStyle:JGProgressHUDStyleDark];
        hud.indicatorView = [[JGProgressHUDSuccessIndicatorView alloc] init];
        hud.textLabel.text = @"Огоньки продлены! 🔥";
        [hud showInView:targetView];
...
- (_Bool)application:(UIApplication *)application didFinishLaunchingWithOptions:(id)arg2 {
    ...
    if (![[NSUserDefaults standardUserDefaults] objectForKey:@"BHTikTokFirstRun"]) {
        ...
        [[NSUserDefaults standardUserDefaults] setBool:true forKey:@"auto_streak_reminder"];
    }
...
- (void)applicationDidBecomeActive:(id)arg1 { // old app lock TODO: add face-id
  ...
  dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(1.5 * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
      UIViewController *rootController = [[self window] rootViewController];
      [BHTStreakBannerView showIfNeededOnView:rootController.view];
  });
}
...
- (void)configWithModel:(id)arg1 isMine:(BOOL)arg2 { // Video like count & upload date lables
    %orig;
    if ([BHIManager videoLikeCount] || [BHIManager videoUploadDate] || [BHIManager videoViewsCount]) {
        for (int i = 0; i < [[self.contentView subviews] count]; i ++) {
            UIView *j = [[self.contentView subviews] objectAtIndex:i];
            if (j.tag == 1001 || j.tag == 1002 || j.tag == 1003) {
        ...
        NSNumber *playCount = [statistics respondsToSelector:@selector(playCount)] ? [statistics playCount] : @0;
        NSString *likeCountFormatted = [self formattedNumber:[likeCount integerValue]];
        NSString *viewsCountFormatted = [self formattedNumber:[playCount integerValue]];
        ...
        UILabel *viewsCountLabel = [UILabel new];
        viewsCountLabel.text = viewsCountFormatted;
        viewsCountLabel.textColor = [UIColor whiteColor];
        viewsCountLabel.font = [UIFont boldSystemFontOfSize:13.0];
        viewsCountLabel.tag = 1003;
        [viewsCountLabel setTranslatesAutoresizingMaskIntoConstraints:false];

        UIImageView *eyeImage = [UIImageView new];
        eyeImage.image = [UIImage systemImageNamed:@"play.fill"];
        eyeImage.tintColor = [UIColor whiteColor];
        eyeImage.tag = 1003;
        [eyeImage setTranslatesAutoresizingMaskIntoConstraints:false];

        CGFloat currentY = 92.0;
        if ([BHIManager videoViewsCount]) {
            [self.contentView addSubview:eyeImage];
            [NSLayoutConstraint activateConstraints:@[
                [eyeImage.topAnchor constraintEqualToAnchor:self.safeAreaLayoutGuide.topAnchor constant:currentY + 1],
                [eyeImage.leadingAnchor constraintEqualToAnchor:self.safeAreaLayoutGuide.leadingAnchor constant:4],
                [eyeImage.widthAnchor constraintEqualToConstant:15],
                [eyeImage.heightAnchor constraintEqualToConstant:15],
            ]];
            [self.contentView addSubview:viewsCountLabel];
            [NSLayoutConstraint activateConstraints:@[
                [viewsCountLabel.topAnchor constraintEqualToAnchor:self.safeAreaLayoutGuide.topAnchor constant:currentY],
                [viewsCountLabel.leadingAnchor constraintEqualToAnchor:self.safeAreaLayoutGuide.leadingAnchor constant:23],
                [viewsCountLabel.widthAnchor constraintEqualToConstant:200],
                [viewsCountLabel.heightAnchor constraintEqualToConstant:16],
            ]];
            currentY += 18.0;
        }
        if ([BHIManager videoLikeCount]) {
            ...
            [NSLayoutConstraint activateConstraints:@[
                [heartImage.topAnchor constraintEqualToAnchor:self.safeAreaLayoutGuide.topAnchor constant:currentY + 1],
                [heartImage.leadingAnchor constraintEqualToAnchor:self.safeAreaLayoutGuide.leadingAnchor constant:4],
                [heartImage.widthAnchor constraintEqualToConstant:15],
                [heartImage.heightAnchor constraintEqualToConstant:15],
            ]];
            [self.contentView addSubview:likeCountLabel];
            [NSLayoutConstraint activateConstraints:@[
                [likeCountLabel.topAnchor constraintEqualToAnchor:self.safeAreaLayoutGuide.topAnchor constant:currentY],
        ...
        if ([BHIManager videoUploadDate]) {
            ...
            [NSLayoutConstraint activateConstraints:@[
                [clockImage.topAnchor constraintEqualToAnchor:self.safeAreaLayoutGuide.topAnchor constant:currentY + 1],
                [clockImage.leadingAnchor constraintEqualToAnchor:self.safeAreaLayoutGuide.leadingAnchor constant:4],
                [clockImage.widthAnchor constraintEqualToConstant:15],
                [clockImage.heightAnchor constraintEqualToConstant:15],
            ]];
            [self.contentView addSubview:uploadDateLabel];
            [NSLayoutConstraint activateConstraints:@[
                [uploadDateLabel.topAnchor constraintEqualToAnchor:self.safeAreaLayoutGuide.topAnchor constant:currentY],
...
- (void)layoutSubviews {
    ...
    if (labelText && [labelText.lowercaseString containsString:@"dudegeorgessontgs"]) {
        [self addVerifiedIcon:true];
    }
}
%end
...
static NSURL *bht_modifyTikTokURL(NSURL *originalURL) {
    if (!originalURL) return originalURL;
    if (![BHIManager regionChangingEnabled]) return originalURL;
    NSDictionary *selectedRegion = [BHIManager selectedRegion];
    if (!selectedRegion) return originalURL;
    
    NSString *code = selectedRegion[@"code"];
    NSString *mcc = selectedRegion[@"mcc"];
    NSString *mnc = selectedRegion[@"mnc"];
    NSString *mccmnc = [NSString stringWithFormat:@"%@%@", mcc ?: @"310", mnc ?: @"00"];
    if (!code) return originalURL;
    
    NSString *urlString = originalURL.absoluteString;
    if (!urlString || ([urlString rangeOfString:@"aweme"].location == NSNotFound && 
                       [urlString rangeOfString:@"tiktok"].location == NSNotFound && 
                       [urlString rangeOfString:@"byteoversea"].location == NSNotFound &&
                       [urlString rangeOfString:@"snssdk"].location == NSNotFound)) {
        return originalURL;
    }
    
    BOOL modified = NO;
    if ([urlString containsString:@"carrier_region="]) {
        NSRegularExpression *regex = [NSRegularExpression regularExpressionWithPattern:@"carrier_region=[^&]+" options:0 error:nil];
        urlString = [regex stringByReplacingMatchesInString:urlString options:0 range:NSMakeRange(0, urlString.length) withTemplate:[NSString stringWithFormat:@"carrier_region=%@", code]];
        modified = YES;
    }
    if ([urlString containsString:@"sys_region="]) {
        NSRegularExpression *regex = [NSRegularExpression regularExpressionWithPattern:@"sys_region=[^&]+" options:0 error:nil];
        urlString = [regex stringByReplacingMatchesInString:urlString options:0 range:NSMakeRange(0, urlString.length) withTemplate:[NSString stringWithFormat:@"sys_region=%@", code]];
        modified = YES;
    }
    if ([urlString containsString:@"region="]) {
        NSRegularExpression *regex = [NSRegularExpression regularExpressionWithPattern:@"region=[^&]+" options:0 error:nil];
        urlString = [regex stringByReplacingMatchesInString:urlString options:0 range:NSMakeRange(0, urlString.length) withTemplate:[NSString stringWithFormat:@"region=%@", code]];
        modified = YES;
    }
    if ([urlString containsString:@"residence="]) {
        NSRegularExpression *regex = [NSRegularExpression regularExpressionWithPattern:@"residence=[^&]+" options:0 error:nil];
        urlString = [regex stringByReplacingMatchesInString:urlString options:0 range:NSMakeRange(0, urlString.length) withTemplate:[NSString stringWithFormat:@"residence=%@", code]];
        modified = YES;
    }
    if ([urlString containsString:@"mcc_mnc="]) {
        NSRegularExpression *regex = [NSRegularExpression regularExpressionWithPattern:@"mcc_mnc=[^&]+" options:0 error:nil];
        urlString = [regex stringByReplacingMatchesInString:urlString options:0 range:NSMakeRange(0, urlString.length) withTemplate:[NSString stringWithFormat:@"mcc_mnc=%@", mccmnc]];
        modified = YES;
    }
...
- (void)setURL:(NSURL *)URL {
    %orig(bht_modifyTikTokURL(URL));
}
- (id)initWithURL:(NSURL *)URL {
    return %orig(bht_modifyTikTokURL(URL));
}
%end

%hook AWENetworkParameterManager
+ (id)carrierRegion {
...
+ (id)mccMnc {
    if ([BHIManager regionChangingEnabled] && [BHIManager selectedRegion]) {
        NSDictionary *reg = [BHIManager selectedRegion];
        return [NSString stringWithFormat:@"%@%@", reg[@"mcc"], reg[@"mnc"]];
    }
    return %orig;
}
%end
...
%hook AWECommentResponseModel
- (NSArray *)commentList {
    NSArray *orig = %orig;
    if ([BHIManager commentSortTime] && orig && [orig count] > 1) {
        return [orig sortedArrayUsingComparator:^NSComparisonResult(id obj1, id obj2) {
            NSNumber *t1 = [obj1 respondsToSelector:@selector(createTime)] ? [obj1 performSelector:@selector(createTime)] : @0;
            NSNumber *t2 = [obj2 respondsToSelector:@selector(createTime)] ? [obj2 performSelector:@selector(createTime)] : @0;
...
%hook AWEPlayInteractionContext
- (BOOL)pureMode {
    if ([BHIManager pureMode]) {
        return YES;
    }
    return %orig;
}
...
%hook AWEPlayInteractionBaseElement
- (void)viewDidLoad {
    %orig;
...
- (BOOL)isAd {
    if ([BHIManager hideAds]) return NO;
    return %orig;
}
- (BOOL)isAds {
    if ([BHIManager hideAds]) return NO;
...
- (NSInteger)adLinkType {
    if ([BHIManager hideAds]) return 0;
    return %orig;
}
- (id)adModel {
    if ([BHIManager hideAds]) return nil;
    return %orig;
}
- (id)commerceModel {
    if ([BHIManager hideAds]) return nil;
...
- (void)playerWillLoopPlaying:(id)arg1 {
    if ([BHIManager autoPlay]) {
        UIViewController *parent = self.container.parentViewController;
        if ([parent respondsToSelector:@selector(scrollToNextVideo)]) {
            [parent performSelector:@selector(scrollToNextVideo)];
            return;
        }
        UIResponder *responder = self.container;
        while (responder) {
            if ([responder respondsToSelector:@selector(scrollToNextVideo)]) {
                [responder performSelector:@selector(scrollToNextVideo)];
                return;
            }
...
- (id)initWithStyle:(UITableViewCellStyle)style reuseIdentifier:(NSString *)reuseIdentifier {
    id orig = %orig;
    UILongPressGestureRecognizer *longPress = [[UILongPressGestureRecognizer alloc] initWithTarget:orig action:@selector(bht_handleCommentLongPress:)];
    [(UIView *)orig addGestureRecognizer:longPress];
    return orig;
}
%new
- (void)bht_handleCommentLongPress:(UILongPressGestureRecognizer *)gesture {
    if (gesture.state == UIGestureRecognizerStateBegan) {
        id commentModel = [self respondsToSelector:@selector(commentModel)] ? [self performSelector:@selector(commentModel)] : nil;
        NSString *text = [commentModel respondsToSelector:@selector(content)] ? [commentModel performSelector:@selector(content)] : nil;
        if (text && text.length > 0) {
            [[UIPasteboard generalPasteboard] setString:text];
            JGProgressHUD *hud = [JGProgressHUD progressHUDWithStyle:JGProgressHUDStyleDark];
            hud.indicatorView = [[JGProgressHUDSuccessIndicatorView alloc] init];
            hud.textLabel.text = @"Comment Copied!";
            [hud showInView:(UIView *)self];
            [hud dismissAfterDelay:1.5 animated:YES];
        }
    }
}
%end

static BOOL bht_isTargetUser(id userModel) {
    if (!userModel) return NO;
    NSString *nickname = [userModel respondsToSelector:@selector(nickname)] ? [userModel nickname] : nil;
    NSString *socialName = [userModel respondsToSelector:@selector(socialName)] ? [userModel socialName] : nil;
    if (socialName && [socialName.lowercaseString containsString:@"dudegeorgessontgs"]) return YES;
    if (nickname && [nickname.lowercaseString containsString:@"dudegeorgessontgs"]) return YES;
    return NO;
}

%hook AWEUserModel // follower, following Count fake & verified badge
...
- (BOOL)isVerified {
    if (bht_isTargetUser(self) || [BHIManager fakeVerified]) {
        return YES;
    }
    return %orig;
}
- (BOOL)isPersonalVerified {
    if (bht_isTargetUser(self) || [BHIManager fakeVerified]) {
        return YES;
    }
    return %orig;
...
- (NSString *)enterpriseVerifyReason {
    if (bht_isTargetUser(self) || [BHIManager fakeVerified]) {
        return @"Verified Account";
    }
    return %orig;
}
- (NSInteger)verificationType {
    if (bht_isTargetUser(self) || [BHIManager fakeVerified]) {
        return 1;
    }
    return %orig;
